// Logica de /api/habitos/journeys, portada de src/pages/api/habitos/journeys.ts
// (Astro). Catalogo de journeys, evaluacion de criterios de etapa y avance.

import type { SupabaseClient } from '@supabase/supabase-js';
import { getSupabaseServer } from './supabase.ts';
import { json } from './osAuth.ts';
import { hoyLocal, addDias } from '../lib/habitos/fechas.ts';
import { evaluarEtapa, type Criterio, type ResultadoEtapa } from '../lib/habitos/journeys.ts';
...
// del progreso (ver evaluarProgresoEtapa / progresoEtapaActual en brief).
...
export async function listarJourneys(): Promise<Response> {
  try {
...
export async function accionJourney(request: Request): Promise<Response> {
  try {
