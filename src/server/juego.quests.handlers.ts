// Logica de /api/juego/quests, portada de src/pages/api/juego/quests.ts (Astro).

import { getSupabaseServer } from './supabase.ts';
import { json } from './osAuth.ts';
import { registrarEvento } from '../lib/juego/motor.ts';
import { evaluarQuest, type ObjetivoQuest } from '../lib/juego/quests.ts';
...
export async function listarQuests(): Promise<Response> {
  try {
    ...
    // procesarLunes en juego.cierre.handlers.ts (una query de xp_events y una de
...
export async function crearQuest(request: Request): Promise<Response> {
  try {
...
export async function cancelarQuest(request: Request): Promise<Response> {
  try {
