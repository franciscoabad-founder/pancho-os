...
// Presets de protocolo de ayuno (Fase 5, estilo Yazio) para la acciÃ³n {protocolo} del
...
// escribibles vÃ­a el passthrough de campos). '16_8' mantiene el formato legacy; '24h' y
// '36h' son tokens nuevos solo de config (al iniciar un ayuno, ayunos.ts los mapea al
// enum de ayunos.protocolo). 'custom' exige objetivo_h explÃ­cito, que se guarda en
// ayuno_objetivo_h (Ãºnica columna nueva, ver 20260720000100).
const PROTOCOLOS_AYUNO = ['16_8', '24h', '36h', 'custom'];
const PROTOCOLO_HORAS: Record<string, number> = { '16_8': 16, '24h': 24, '36h': 36 };

// Devuelve (y crea si no existe) la fila Ãºnica de salud_config con los targets.
...
export const PATCH: APIRoute = async (context) => {
  ...
  try {
    ...
    // sincroniza las horas en ayuno_objetivo_h. 'custom' exige objetivo_h explÃ­cito (no
    // hay default razonable). Este PATCH vive acÃ¡ (y no en ayunos.ts) porque el
    ...
    if ('protocolo' in body) {
      ...
      if (protocolo === 'custom') {
        ...
        if (!Number.isFinite(objetivo) || objetivo <= 0) {
          return json({ error: 'protocolo custom requiere objetivo_h numÃ©rico' }, 400);
        }
      ...
      const objetivo = typeof body.objetivo_h === 'number' ? body.objetivo_h : Number(body.objetivo_h);
      if (!Number.isFinite(objetivo) || objetivo <= 0) {
        return json({ error: 'objetivo_h invÃ¡lido' }, 400);
      }
      patch.ayuno_objetivo_h = objetivo;
    }
