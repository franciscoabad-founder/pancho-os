// Server route de logros de GFIT, portado de src/pages/api/gfit/logros.ts
// (Astro) a TanStack Start. Mismo molde que progreso.ts.
// gfit_consultar_progreso (src/mcp/osTools.ts) depende de GET
// /api/gfit/logros.
//
...
import { sugerirProgresion, type SerieReciente } from '../../../lib/gfit/progresion.ts';
import { evaluarLogros, type ContextoLogros, type CatalogoLogro, type SerieConGrupo } from '../../../lib/gfit/logros.ts';
...
export const Route = createFileRoute('/api/gfit/logros')({
  server: {
    handlers: {
      GET: async ({ request }) => {
