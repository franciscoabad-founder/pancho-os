// Logica de /api/salud/recetas, portada de src/pages/api/salud/recetas.ts.
// Recetas propias e importadas desde una URL con JSON-LD (schema.org/Recipe).
...
export async function leerRecetas(params: {
  id: string | null;
  q: string;
  fuente: string;
  tagsParam: string;
  ...
  if (params.id) {
    const { data: receta, error } = await sb.from('recetas').select('*').eq('id', params.id).single();
  ...
  const limit = Math.min(200, Math.max(1, numOrNull(params.limitParam) ?? 40));
  const offset = Math.max(0, numOrNull(params.offsetParam) ?? 0);

  let query = sb.from('recetas').select('*').order('nombre', { ascending: true });
  if (params.q) query = query.ilike('nombre', `%${params.q}%`);
  if (params.fuente) query = query.eq('fuente', params.fuente);
  if (params.tagsParam) {
    const tags = params.tagsParam.split(',').map((t) => t.trim()).filter(Boolean);
...
/** Rama de importacion: POST { importar_url }. Un fallo de parseo responde 422. */
export async function importarReceta(sourceUrl: string) {
  let importada: RecetaImportada;
  try {
    importada = await importarRecetaDeUrl(sourceUrl);
  } catch (err) {
    throw new ErrorHttp(422, errMsg(err));
  }
  const sb = getSupabaseServer();
  ...
  return { receta, ingredientes_importados: importada.ingredientes.length };
}

export async function crearReceta(body: Record<string, any>) {
...
export async function actualizarReceta(id: string, body: Record<string, any>) {
