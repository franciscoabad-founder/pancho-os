...
- F Agenda: corrección lista localmente y pendiente de migración/deploy. `/agenda` muestra catorce días consecutivos en `America/Guayaquil`, incluso días vacíos, y permite crear, editar, borrar y etiquetar eventos. La sincronización bidireccional conserva cambios locales pendientes, cancela en Google los borrados locales y usa metadatos privados para etiquetas. Producción confirmó que aún no tiene las columnas `google_*`/`etiquetas` ni OAuth. El runbook exacto está en `docs/agenda-google-calendar-runbook.md`; no desplegar hasta aplicar `20260829000000_agenda_editable_sync.sql`.
