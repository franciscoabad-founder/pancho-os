// Logica de /api/habitos/checks, portada de src/pages/api/habitos/checks.ts (Astro).
// Registra y deshace checks de habitos, con recompensa (motor de juego) y racha.

import { getSupabaseServer } from './supabase.ts';
import { json } from './osAuth.ts';
import { hoyLocal } from '../lib/habitos/fechas.ts';
import { nuevoValor, recompensaCheck, type Dificultad } from '../lib/habitos/scoring.ts';
...
export async function registrarCheck(request: Request): Promise<Response> {
  try {
...
export async function deshacerCheck(request: Request): Promise<Response> {
  try {
    const url = new URL(request.url);
    let habitoId = url.searchParams.get('habito_id');
