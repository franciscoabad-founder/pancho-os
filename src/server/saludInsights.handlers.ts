// Logica de /api/salud/insights, portada de src/pages/api/salud/insights.ts.
//
// Coach de Salud: seis generadores independientes (comidas, ayuno, entreno,
// habitos, targets, sueno) que se ejecutan en paralelo y cuyos fallos se tragan
// individualmente, para que una tabla ausente no deje al coach mudo.
// Ver docs/contrato-insights-coach.md.

import type { SupabaseClient } from '@supabase/supabase-js';
import { getSupabaseServer } from './supabase.ts';
import { hoyGuayaquil } from '../lib/salud/apiHelpers.ts';
import { addDias } from '../lib/habitos/fechas.ts';
import { falloAyer } from '../lib/habitos/racha.ts';
import { calcularDeuda, formatearHoras, type NocheSueno } from '../lib/sueno/deuda.ts';
...
/** { generado_at, insights } con maximo 6 entradas, alerta > nudge > info. */
export async function generarInsights(): Promise<{ generado_at: string; insights: Insight[] }> {
  ...
  return { generado_at: new Date().toISOString(), insights };
}
